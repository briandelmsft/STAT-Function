...
def execute_gpt_module (req_body):
    ...
    # When asked, respond exclusively in the following JSON format {"Summary": "", "Actions": [{"ActionName": "", "ActionDescription": ""}]}
...
def base_module_messages(gpt_object:GPTModule, base_object:BaseModule):
    ...
    if base_object.Tactics:
        add_message(gpt_object, 'assistant', f'This incident is linked to the MITRE Tactics {", ".join(base_object.Tactics)}')

    if base_object.Techniques:
        add_message(gpt_object, 'assistant', f'This incident is linked to the MITRE Techniques {", ".join(base_object.Techniques)}')
